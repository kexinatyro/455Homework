    ...
    for(i = 0; i < m.rows; ++i){
        ...
        for(j = 0; j < m.cols; ++j){
            ...
            if(a == LOGISTIC){
                ...
                // result: pow(M_E, x) normalize later
                m.data[i][j] = exp(x);
            }
            sum += m.data[i][j];
        }
    ...
    ************************************************************************/
    ...
    for(i = 0; i < m.rows; ++i){
        for(j = 0; j < m.cols; ++j){
            ...
            // activation is LOGISTIC
            if(a == LOGISTIC){
                // d[i,j] = x(1-x)d[i,j]
                d.data[i][j] *= x * (1 - x);
            // activation is RELU
            } else if (a == RELU){
                // if x <= 0: d[i,j] = 0 or d[i,j]
                if (x <= 0) {
                    d.data[i][j] = 0;
                }
            // activation is LRELU
            } else if (a == LRELU){
                // if x <= 0: d[i,j] /= 10 or d[i,j]
                if (x <= 0) {
                    d.data[i][j] /= 10;
                }
            }
...
matrix forward_layer(layer *l, matrix in)
{
    ...
    ************************************************************************/
    ...
    // mutiply matrix in and l->w
    matrix out = matrix_mult_matrix(in, l->w);
    // apply activate_matrix function
...
matrix backward_layer(layer *l, matrix delta)
{
    ...
    ************************************************************************/
    ...
    // add the activation into consideration for matrix delta
    ...
    // make tranpose of in
    matrix inTranspose = transpose_matrix(l->in);
    // multiply transpose in and delta
    matrix dw = matrix_mult_matrix(inTranspose, delta);
    l->dw = dw;
    ...
    // make transpose of w
    matrix wTranspose = transpose_matrix(l->w);
    // multiply delta and w transpose
    matrix dx = matrix_mult_matrix(delta, wTranspose);
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    ************************************************************************/
    ...
    // Update l->w
    matrix delta_w_t1 = axpy_matrix(rate, delta_w_t, l->w);
    // Remember to free any intermediate results to avoid memory leaks
    free_matrix(l->w);
    //save
    l->w = delta_w_t1;
